const pontos = [
...

    {
        ...
        "fotos2": "/static/img/cachoeira_urubu_rei2.png",
        "fotos3": "/static/img/cachoeira_urubu_rei3.png",
...
// Organiza os pontos por categoria
pontos.forEach(ponto => {
    if (categorias[ponto.categoria]) {
        categorias[ponto.categoria].push(ponto);
    }
});
...
Object.keys(categorias).forEach(categoria => {
    if (categorias[categoria].length > 0) {
        ...
        tituloCategoria.classList.add('categoria-explorer');
        tituloCategoria.textContent = categoria;
        containerPrincipal.appendChild(tituloCategoria);

        // Cria a estrutura do álbum
        const albumDiv = document.createElement('div');
        albumDiv.classList.add('album', 'py-5', 'bg-body-tertiary', 'flex');

        const containerDiv = document.createElement('div');
        containerDiv.classList.add('container');

        const rowDiv = document.createElement('div');
        rowDiv.classList.add('row', 'row-cols-1', 'row-cols-sm-2', 'row-cols-md-3', 'g-3');

        categorias[categoria].forEach(ponto => {
            ...
            link.href = `/paginas/ponto.html#${ponto.id}`;
            link.style.textDecoration = 'none';

            const img = document.createElement('img');
            img.src = ponto.fotos1;
            img.dataset.src = ponto.fotos1;
            img.classList.add('d-block', 'w-100', 'imgExplorer', 'lazyload', 'lazyloading', 'lazyloaded');
            ...
            descricao.innerHTML = `${ponto.descricao_resumida} <br>`;
            ...
            smallTime.innerHTML = `
                ...
                ${ponto.tempo_passeio} horas`;

            descricao.appendChild(smallTime);

            const infoDiv = document.createElement('div');
            infoDiv.classList.add('d-flex', 'justify-content-between', 'align-items-center');

            const smallDistancia = document.createElement('small');
            smallDistancia.classList.add('text-body-secondary');
            smallDistancia.innerHTML = `
            ...
            const smallDificuldade = document.createElement('small');
            smallDificuldade.classList.add('text-body-secondary');
            smallDificuldade.textContent = ponto.grau_dificuldade;

            const smallMais = document.createElement('small');
            smallMais.classList.add('text-body-secondary');
            smallMais.textContent = "Clique Para Saber Mais.";

            infoDiv.appendChild(smallDistancia);
            infoDiv.appendChild(smallDificuldade);
            infoDiv.appendChild(smallMais);
        ...
        containerPrincipal.appendChild(albumDiv);
    }
});
